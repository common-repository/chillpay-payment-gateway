...
<fieldset id="chillpay-form-ewallet">
    <ul style="list-style:inherit">
    	<?php if ( $currency == 'THB') { ?>	
			...
			<!-- Paotang Pay -->
			<?php if ( $paotangCards ) : true ?>
				<li class="item">
					<input id="epayment_paotang" type="radio" name="chillpay-offsite" value="epayment_paotang" <?php if($viewData['fee_paotang'] == -1) { echo 'disabled'; } ?> />
					<label for="epayment_paotang">
						<div class="chillpay-form-ewallet-logo-box">
							<img class="<?php if($viewData['fee_paotang'] == -1) { echo 'opacity-1'; } ?>" src="<?php echo plugins_url( '../../assets/images/icon-epayment/Icon_PaotangPay.png', __FILE__ ); ?>" style="max-height:100%;width:38px;height:100%;"/>
						</div>
						<div class="chillpay-form-ewallet-label-box" style="width:100%;">
							<span class="title <?php if($viewData['fee_paotang'] == -1) { echo 'opacity-1'; } ?>"><?php _e( 'Paotang Pay', 'chillpay' ); ?></span><br/>
							<?php if ( $viewData['fee_paotang'] == -1 ) : ?>
								<span class="rate secondary-text" id="rate_paotang" >
									Channel not available
								</span>
							<?php else : ?>
								<span class="rate secondary-text" id="rate_paotang">
									<?php echo $viewData['fee_paotang'] ?>
								</span>
							<?php endif; ?>
						</div>
					</label>
