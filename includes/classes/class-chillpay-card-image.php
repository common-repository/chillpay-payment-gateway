...
if ( ! class_exists( 'ChillPay_Card_Image' )) {
    class ChillPay_Card_Image {
		...
		/**
		 * Return the default setting of display the Paotang Pay logo
		 *
		 * @return string
		 */
		public static function get_paotang_default_display() {
			return 'yes';
		}


		/**
		 * Return the HTML &lt;img&gt; element of Paotang Pay logo
		 *
		 * @return string
		 */
		public static function get_paotang_image() {
			return self::get_image( 'icon-epayment/Icon_PaotangPay.png', 'Paotang Pay' , '38px', null);
		}
		...
		/**
		 * Check whether the setting for Paotang Pay logo is configured and it was set to display or not display
		 *
		 * @param mixed $setting The array that contains key for checking the flag
		 * @return boolean
		 */
		public static function is_paotang_enabled( $setting ) {
			// Make it backward compatible. If the setting is not configured, the Paotang Pay logo is display by default.
			if ( ! isset( $setting['accept_paotang'] ) ) {
				return self::get_paotang_default_display();
			}

			if ( $setting['accept_paotang'] == 'yes' ) {
				return true;
			}
