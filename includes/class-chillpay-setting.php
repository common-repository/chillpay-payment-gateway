...
class ChillPay_Setting
{
	...
	public function APIFee_V2()
	{
		...
		unset($this->ewallet_fees['epayment_paotang']);
		$this->ewallet_fees['epayment_paotang'] = array();
		...
		for ($i = 0; $i < $count; $i++) {
			...
			} else if (strcmp($code, 'epayment_paotang') === 0) {
				$array_ewallet_paotang['fee_amount'] = $fee_amount;
				$array_ewallet_paotang['fee_min_amount'] = $fee_min_amount;
				$array_ewallet_paotang['fee_type'] = $fee_type;
				$array_ewallet_paotang['payment_min_price'] = $payment_min_price;
				$array_ewallet_paotang['payment_max_price'] = $payment_max_price;
				$array_ewallet_paotang['currency_code'] = $currency_code;
				$array_ewallet_paotang['currncy_name'] = $currncy_name;

				if ($this->ewallet_fees['epayment_paotang'] == array()) {
					$this->ewallet_fees['epayment_paotang'] = array($array_ewallet_paotang);
				} else {
					array_push($this->ewallet_fees['epayment_paotang'], $array_ewallet_paotang);
				}			
