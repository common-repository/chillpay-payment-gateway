...
class ChillPay_Payment_eWallet extends ChillPay_Payment
{
    ...
    {       
        ...
        $this->channel			  = __('Alipay, Rabbit LINE Pay, ShopeePay, TrueMoney Wallet, WeChat Pay, Paotang Pay');
    ...
    public function init_form_fields()
    {
        ...
        $paotang = array(
            'accept_paotang' => array(
                'type'        => 'checkbox',
                'label'       => ChillPay_Card_Image::get_paotang_image(),
                'css'         => ChillPay_Card_Image::get_css(),
                'default'     => ChillPay_Card_Image::get_paotang_default_display(),
                'description' => 'Paotang Pay'
            ),
        );
        $this->form_fields = array_merge($this->form_fields, $paotang);
    }
    ...
    public function payment_fields()
    {
        ...
        $card_icons['accept_paotang'] = $this->get_option( 'accept_paotang' );
		...
		if (isset($fee_ewallet)) {
            ...
            $fee_paotang = $fee_ewallet['fee_paotang'];
		}
        ...
        if (ChillPay_Card_Image::is_paotang_enabled($card_icons)) {
            $viewData["paotang"] = true;
            $viewData["fee_paotang"] = -1;
            if($fee_paotang >= 0) {
				$viewData["fee_paotang"] = "Fee : " . number_format((float)$fee_paotang, 2) . " " . $this->currency_support;
			}
        }
